...
OUT_V3="runs/sealed_ci_v3"
rm -rf "$OUT_V1" "$OUT_V2" "$OUT_V3"
...
uv run sovereidolon suite run-sealed --suite-file examples/sealed/sealed_v3.json --out-dir "$OUT_V3"
uv run sovereidolon store audit --store "$OUT_V3/store"
test -f "$OUT_V3/sealed_summary.json"
