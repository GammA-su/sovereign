...
def test_suite_v15_scale_lite_baseline(tmp_path: Path) -> None:
    runner = CliRunner()
    out_dir = tmp_path / "suite_v15"
    result = runner.invoke(
        ...
        [
            ...
            "run",
            "--suite-file",
            "examples/suites/suite_v15_scale_lite.json",
            "--out-dir",
            str(out_dir),
        ],
    )
    ...
    baseline = read_json(
        Path("examples/baselines/suite_v15_scale_lite.report.norm.json")
    )
